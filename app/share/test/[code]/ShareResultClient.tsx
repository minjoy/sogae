...
import { TEST_INFO } from '@/lib/share-code';
...
export function getTypeDetail(testId: number, primaryLabel: string) {
  return TYPE_DESCRIPTIONS[testId]?.[primaryLabel];
}
...
export default function ShareResultClient({ result }: ShareResultClientProps) {
            ...
            {Object.entries(TEST_INFO).map(([id, info]) => (
              ...
              >
                ...
                <div className="text-xs text-gray-600 leading-tight">
                  {info.title.replace(' 테스트', '').replace(' 심리', '').replace(' 방식', '')}
                </div>
                {parseInt(id) === result.t && (
